...
In this standard form, it is generally assumed that unit conversion can be performed using a multiplicative conversion factor. This is true for all SI quantities except when thermodynamic temperature or temperature on the ITS-90 scale is expressed in degrees Celsius (temperature difference or uncertainty expressed in degrees Celsius can, however, be converted using a multiplicative factor). A related assumption is that unit-expressions can be simplified according to the rules of multiplication and division (e.g., $\si{\metre\cubed}/\si{\metre\squared}= \si{\metre}$). 

These assumptions do not hold when temperature is expressed in degrees Celsius, because \SI{0}{\celsius} is offset from \SI{0}{\kelvin}. In general, situations like this will arise if the zero point shifts during unit conversion.  
...
Measurement system calibration equations often describe corrections for system readings. Care must be taken to avoid misinterpretation when units like the degree Celsius are involved, because multiplicative unit conversion does not apply. In such cases, coefficient values should be reported as pure numbers and a description of terms provided as text.

For example, the following equation appears in some of MSL's calibration reports for electronic thermometers. The correction,
\[
\Delta t = A + B \cdot R + C\cdot R^2 + D\cdot R^3\;,
\]
is added to a thermometer indication, $R$. Reports should associate units with indications (which relates to the instrument configuration) but $A, B, C,\text{ and }D$ should be reported as pure numbers. For example, coefficient values and a possible sentence to accompany the equation might appear as
\begin{align*}
    A &= \num{-1.05e-2}\\
    B &= \num{+4.80e-6}\\
    C &= \num{+1.19E-8}\\
...
\end{align*}
\begin{quote}
\textit{When the thermometer is configured to indicate in degrees Celsius, the correction, $\Delta t$, can be added to the reading, $R$, to obtain a value of temperature expressed in degrees Celsius. }
\end{quote}
