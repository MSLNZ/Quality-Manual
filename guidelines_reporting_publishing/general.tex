...
When a quantity is expressed as a number followed by a unit symbol, it is also often assumed that unit conversion can be performed with a multiplicative conversion factor. This is true for all SI units except when the degree Celsius is used to express absolute temperature or temperature on the ITS-90 scale (temperature difference or uncertainty expressed in degrees Celsius can, however, be converted by a multiplicative factor). A related assumption is that unit-expressions can be simplified according to the rules of multiplication and division (e.g., $\si{\metre\cubed}/\si{\metre\squared}= \si{\metre}$). These assumptions are certainly not correct when temperature is expressed in degrees Celsius and may not be justified for other non-SI units.

Calibration equations reported by MSL must take care to avoid ambiguity or misinterpretation associated with unit symbols like the degree Celsius. Calibration equations usually describe how to calculate a correction to be applied to an indicated value, and include coefficient values determined by MSL. For example, this appears in some of MSL's Temperature reports for electronic thermometers. The correction,
\[
\Delta t = A + B \cdot R + C\cdot R^2 + D\cdot R^3\;,
\]
would be added to the indication, $R$, and the values of $A, B, C,\text{ and }D$ are determined during calibration. To avoid misinterpretation, reports should specify indicated units but $A, B, C,\text{ and }D$ should be reported as pure numbers. For example,
\begin{align*}
    A &= \num{-1.05e-2}\\
    B &= \num{+4.8e-6}\\
    C &= \num{+1.19E-8}\\
