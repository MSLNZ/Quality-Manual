...
\end{center}
\item The first person can now `sign' the report by inserting an image of their signature using PDF reader software (~see \cite[\S\ref*{GRP-ss:electronic_reports}]{MSL_Reporting_Guidelines}~). \deprecated{The person who signed must upload the new version to EDI.}

Note,
\begin{itemize}
\item the file name must not be changed \deprecated{(EDI maintains a version history based on file name)}
...
\item If corrections to the report are necessary during the signing process, the faulty version can be annotated with comments, highlighted sections, etc (using a PDF reader), \deprecated{and uploaded to EDI}. Doing so maintains a full record of the signing/issuing process. 

Once corrected, the new report file must be signed again by everyone.
\end{enumerate}

This process generates a version history similar to the one below. All intermediate versions can be reviewed by clicking on the links at the left. Note the checksum in the comment associated with the final version. \deprecated{Figure needs replacing}
...
\item For the central file copy, the name of the file should not be changed. Rather, the cancelled file should be saved with the same name and a note added about the cancellation in the \deprecated{EDI version history of the document}. 
\end{itemize}
...
\item \deprecated{A note should be added in the EDI version history} to indicate a reissued report.
\item The metadata tag `reissued' for \deprecated{the EDI document} should be set.
\end{itemize}
...
\begin{itemize}
\item For a PDF file, add the text “{\color{red}CANCELLED}” to each page (in red text, with bold, large, font size – there are PDF reader programmes that can do this). 
\item For the central file copy, the name of the file should not be changed. Rather, the cancelled file should be saved with the same name and a note added about the cancellation in \deprecated{the EDI version history} of the document. 
\end{itemize}
...
\begin{itemize}
\item The name of the central file copy should not be changed (i.e., the new copy of the report replaces the cancelled original). 
\item Add a note to the \deprecated{EDI version history} to indicate a reissued report.
\item Set the \deprecated{EDI document metadata} tag `reissued'.
\end{itemize}
\end{enumerate}
...
\item Add a note to indicate a replaced report in \deprecated{the EDI version history}.
\item Set the \deprecated{EDI document metadata} tag `reissued'.
\end{itemize}
\end{itemize}
...
\item Do not change the file name in the central file. Rather, the cancelled file should be saved with the same name and a note added to \deprecated{the EDI version history} of the document about the cancellation. 
\end{itemize}
\end{enumerate}
...
\begin{itemize}
\item The name of the central file copy should be the new file name. (The old file name may be changed \deprecated{on EDI} without breaking the version history. Seek help from the Quality Manager or site administrator to do this before uploading the new version of the file.) 
\item The central file metadata tag `reissued' should be set.
\item Add a note to the \deprecated{EDI version history} to indicate a replaced report.
\end{itemize}
...
The MSL Publications library is the common repository for publications. It is part of the \deprecated{`Measurement Standards Laboratory' site on EDI}. Various categories of document can be stored there, including: 
\begin{itemize}
...
A Publication Clearance Form can be used to record the review of a publication in the repository. The form is one of \deprecated{the `new' document types that can be created inside the docset (`bucket') associated with the publication}.
...
The \deprecated{Publications Library} should be used to store the final manuscript draft, and a record of MSL staff approval for submission. Other information, such as the agreement between authors, may also be saved in the library. 
