...
\begin{center}
\includegraphics[scale=.75]{pictures/metadata_signed_report}
\end{center}
\item The first person can now `sign' the report by inserting an image of their signature using PDF reader software (~see \cite[\S\ref*{GRP-ss:electronic_reports}]{MSL_Reporting_Guidelines}~). In AODocs, this can be done by the signatory either downloading the PDF, signing and then uploading to the same file name, OR by using the AODocs Universal File Opener (UFO) browser extension.  The UFO is the more efficient solution with guidance available through the AODocs MSL Quality Management System home page.

Note,
\begin{itemize}
\item the file name must not be changed (AODocs maintains a version history based on file name)
...
%They should organise a reviewer and make sure that all relevant documents are stored in the publications library. 
...
\proposed{\item Externally published material (e.g., papers in scientific journals) is always subject to the terms of a publishing agreement, between the publisher and authors. A copy of this agreement must be stored with the other documents in the library.} 
...
\proposedbox{
\subsection{Open data policy}
\label{ss:open_data_policy}
MSL promotes good open science research practices.

Researchers will publish technical documents in an open and transparent manner unless confidentiality of information or research findings prevent this. Scientific papers will be made available according to open-access principles, to the extent that copyright or other agreements with publishers or collaboration partners allows this.
 
Whenever possible, MSL will retain the copyright to work products, and use licences that enhance reuse.
 
MSL aims to provide open access to data appearing in, or underpinning, scientific papers and Technical Reports. Access to data will be as open as possible and as closed as necessary, the balance being determined for each project.

Data management and data access will follow Findable Accessible Interoperable Reusable (FAIR) principles.

{\small\textit{(This policy is owned by the Chief Metrologist)}}

\subsubsection{Types of open-access publication}
The term \textit{open access} means that a publication is made available online, at no cost to the reader, and with few restrictions on reuse. 

Publications may be opened in different ways. Two common distinctions are:
\begin{description}
    \item[Gold or hybrid Open Access: ] articles and related data are accessible on the journal's website (the author usually pays a fee to the publisher).
    \item[Green Open Access: ] articles are accessible on a personal website, a website operated by their funding body, or an independent scientific repository. (there is usually no charge to the author). Green open access conditions vary quite a lot between publishers. They will be specified in a publishing agreement (see \S\ref{sss:pub_agreements}).  
\end{description} 

\textit{Self-archiving} may be used to describe green open access. Self-archiving is when an author provides open access to a document online (usually the document is a postprint uploaded to an open archive platform for the purpose of maximizing its accessibility). Self-archiving applies to peer-reviewed research journal and conference articles, as well as theses and book chapters. 
...
A publication agreement will clearly state the conditions under which a publication can be opened. 

Such agreements may distinguish between manuscripts at three stages of the review process (the terms used by different publishers vary).
\begin{enumerate}
\item a \textbf{\textit{preprint}} is the original manuscript submitted, prior to peer-review; 
\item a \textbf{\textit{postprint}}, or \textbf{\textit{accepted manuscript}}, is the accepted manuscript before typesetting and final editorial tweaks by the journal;
\item a \textbf{\textit{version of record}} is the final published manuscript, typeset by the journal. 
\end{enumerate}

Authors retain ownership of preprints. However, publishers may place restrictions on accepted manuscripts as well as the version of record. The restrictions can vary, so the publication agreement must be followed. 

Usually when accepted manuscripts are opened by self-archiving, an explicit link must be given to the version of record on the publisher's website. An embargo period after publication may also apply before an accepted manuscript can be opened.

Two of the creative commons licences are typically used to open a publication (publishers adopt different policies): \href{https://creativecommons.org/licenses/by-nd/4.0/}{CC BY 4.0}, the Attribution 4.0 International licence, and \href{https://creativecommons.org/licenses/by-nd/4.0/}{CC BY-ND 4.0}, the Attribution-NoDerivatives 4.0 International licence. 

\subsubsection{Repositories}
\label{sss:repositories}
The repository preferred for MSL's scientific and technical publications is \href{https://zenodo.org/}{Zenodo}, which is operated by CERN. MSL Technical Guides and Technical Reports must be deposited in Zenodo (more details required!).  

When science projects are externally funded, the funding body may prescribe rules for the open access of work products. It is important to consider these before preparing publication material.
...
Every MSL author must have an ORCID. Authors of MSL Technical Guides and Technical Reports must ensure that their ORCIDs appear on the documents.

Every MSL Technical Guide or Technical Report must have a DOI, which must be clearly visible on the document. Zenodo can issue DOIs for all archived material.
}
...
\begin{itemize}
\item  Follow the points listed above for scientific papers.
\item  \deprecated{Consider publishing under a Creative Commons licence (~see \cite[\S\ref*{GRP-s_copyright}]{MSL_Reporting_Guidelines}~).}
...
\begin{itemize}
\item \proposed{github repository}
\item \deprecated{Templates for Technical Guides are kept with the current guide documents on the G-drive} (look for: \verb|G:\Shared drives\MSL - Private\Technical Guides|).
\item MSL Technical Guides should be readily identifiable with MSL. They should display the MSL logo on the front page and include generic contact information (email, www). 
...
\item  Make sure an MSL technical guide number has been allocated.
\item  A version number and date of publication should appear on the front page.
\item  The person responsible for the Technical Guide should be identified, but not their direct contact details.
\item  Review for technical correctness.
...
\item \proposed{Deposit in Zenodo}
\end{itemize}
