...
\proposedbox{\subsubsection{Issuing Electronic Reports and Summary Data}
The electronic report file shall be signed using the procedure described in \S\ref{sss:electronic_signatures}, which will ensure that a copy is saved in the central file system ( see~\S\ref{ss:central_file_system} ). 

An electronic copy of the data and documents used to create the report file shall be retained in the section file system ( see~\S\ref{ss:section_file_systems} ). 
   
The client will be supplied with a copy of the report and the MD5 checksum for the report file ( see~\ref{} ).

Clients may request other calibration information in electronic format, such as an Excel spreadsheet. Such information may be provided, but only after the report has been issued. The electronic data provided must be checked with the same care given to written reports. Moreover, it should be made clear to the client that the report remains the authoritative document. 
}
...
\begin{enumerate}
\item If a paper report was issued, contact the client and ask for the return of the original report and also that any copies be destroyed. If an electronic report was issued, ask the client to destroy all copies.
...
\item For the central file, the file name should not be changed. Rather, the cancelled file should be saved with the same name and a note added to the EDI version history of the document about the cancellation. 
\end{itemize}
\end{enumerate}
...
\item Set the central file metadata tag `reissued' for the EDI document.
