...
\deprecatedbox{\subsubsection*{Issuing Reports and Summary Data}}
\proposedbox{\subsubsection{Issuing Paper Reports and Summary Data}}
...
\item Before deciding to issue a report, ensure that the client is happy to receive a report in electronic format.
\item The electronic report file shall be signed following the procedure described in \S\ref{sss:electronic_signatures}, which will ensure that a copy is saved in the central file system ( see~\S\ref{ss:central_file_system} ). 

\item An electronic copy of the data and documents used to create the report file shall be retained in the section file system ( see~\S\ref{ss:section_file_systems} ). 
   
\item The client will be supplied with a copy of the report and the MD5 checksum for the report file (see ``Checking the integrity of a report file'', in MSL Guidelines on reporting and publishing \cite[\S\ref*{GRP-ss:file_integrity_md5}]{MSL_Reporting_Guidelines}).

The following message should be communicated when providing the report to the client (with appropriate replacements of \textit{<report ID>} and  \textit{<checksum>})
\begin{quote}
...
The section ``Electronic reports'' of the MSL Guidelines on Reporting and Publishing contains information on how to produce a calibration report file with a suitable MSL report cover and how to insert electronic signature images in that file \cite[\S\ref*{GRP-ss:electronic_reports}]{MSL_Reporting_Guidelines}.

Signing must be carried out according to the following procedure, which is intended to create a record of the process.

\begin{enumerate}
\item Create a PDF version of the report file
\item Upload the PDF file to the ``Calibration Reports'' library in EDI. Note, the file name should be in the following format:
\begin{quote}
...
\end{center}
\item The first person can now `sign' the PDF report file by inserting an image of their signature with PDF reader software. 
...
Before uploading the final version, signed by everyone, an MD5 checksum is calculated (see section ``Checking the integrity of a report file'' in the MSL Guidelines for reporting and publishing \cite[\S\ref*{GRP-ss:file_integrity_md5}]{MSL_Reporting_Guidelines}). 
...
\deprecated{To withdraw a report} 



\proposed{

\paragraph{To withdraw paper reports}\mbox{}}
\begin{enumerate}
...
\item Any change of information in the report must be clearly identified and, where appropriate, the reason for the change should be included in the report \proposed{(some advice on formatting is given in the `Reissued reports' section of the Guidelines on Reporting and Publishing \cite[\S\ref*{GRP-ss:reissued_reports}]{MSL_Reporting_Guidelines} ).}
...
\proposedbox{\paragraph{To withdraw electronic reports}
\begin{enumerate}
...
\item Reissue the corrected report and a copy of the cancelled original report to the client. Include an MD5 checksum for the corrected report.
...
\paragraph{Note} changes to the central file should only be carried out by an MSL administrator or the Quality Manager.
...
\proposed{\paragraph{Electronic reports} no special procedure is required to replace an electronic report. Simply dispatch a copy of the report to the client, together with the corresponding MD5 checksum.

There is no need to record the replacement of the report in the central file system.}

\proposed{
\paragraph{Paper reports}\mbox{}}
...
\proposed{Note, the client, on request, may be supplied with a copy of an original paper report, in which case the copy is not considered ``reissued". However, the client should be advised that the copy is not authoritative and, for example, may not be acceptable in court.}
