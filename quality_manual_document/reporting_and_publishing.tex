...
\proposedbox{
This section describes general MSL policy for publications (other than the measurement reports described in \S\ref{ss:metrological_reports}). The policy is intended to mitigate reputational risk to MSL by ensuring consistently high publication standards.   



In general, it is intended that
\begin{itemize}
\item  Every document should be reviewed before publication, and some record of that should be kept. 

Sometimes a publication may not seem sufficiently important to warrant a formal review. However, our experience suggests that the review process is always beneficial.
(The potential for harm to occur (risk), if poor quality material is made public, can be considered when deciding how much care is needed to review a document.)  

\item  Documents for publication will be made available in a common repository (~see~\S\ref{sss:publications_repository}~)

\item  A record of approvals will be kept with the material for publication in the repository 

\item One of the MSL authors should coordinate the review process. They should organise a reviewer and also make sure that all relevant documents are stored in the repository. 
...
Notes: 
\begin{itemize}
\item  The record of a review (in the repository) may be quite detailed or contain sufficient information to locate such detail elsewhere (lab books, spreadsheet on the \deprecated{I-drive}~\proposed{G-drive}, etc), or it may simply state that the work has been viewed and is of publication quality.  
