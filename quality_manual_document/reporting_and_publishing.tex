...
\proposed{\item Externally published material (e.g., papers in scientific journals) is always subject to the terms of an agreement between the publisher and the authors. A copy of this agreement must be stored with the other documents in the library.} 
...
Open access means that a publication is made available online, at no cost to the reader, and with few restrictions on reuse. 

Publications may be opened in different ways.
\begin{description}
    \item[Gold or hybrid Open Access: ] articles and related data are made accessible on the journal's website (the author usually pays a fee to the publisher).
    \item[Green Open Access: ] articles are made accessible on a personal website, a website operated by their funding body, or an independent repository. There is usually no charge to the author. However, conditions will be specified in the publishing agreement signed when a manuscript is accepted for publication (see \S\ref{sss:pub_agreements}).  
\end{description} 

\subsubsection{Publishing agreements and the review of manuscripts}
...
\item The manuscript accepted for publication at the end of the review process, but before typesetting and final editorial tweaks by the journal, is called an \textit{accepted manuscript} (AM)
\item The published paper, typeset by the journal, is called a \textit{version of record} (VOR))
\end{enumerate}

Authors always retain full rights over preprints, which can be made available on a personal website or an independent repository at any time. 

When gold open access is involved, the VOR is usually made available. 

When green open access is involved, the publication agreement will clearly state the conditions under which a publication can be opened. Usually the AM version is made available and there may be an embargo period before it can be released. The publisher usually requires an opened AM document to be annotated with a link pointing to the journal website and the VOR. 

In both gold and green open access, the creative commons license \href{https://creativecommons.org/licenses/by-nd/4.0/}{CC BY-ND 4.0} is usually applied.
}
