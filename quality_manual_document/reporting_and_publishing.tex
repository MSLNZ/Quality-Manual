...
\begin{enumerate}
\item Create a PDF/A 3a version of the report file. From MSWord, this can be done by selecting ``PDF'' from the ``Save as type'' menu.  (See note 2 immediately below for how to configure MSWord to make PDF/A 3a the default PDF version.)

The file name should be in the following format:
\begin{quote}
\texttt{<section name>\_<section report number>\_<year>.pdf}
\end{quote}
...
Note 2:  The default configuration within MSWord is for there to be no PDF standards applied to the generation of PDF files.  This can be changed by following these steps \\
\begin{itemize}
\item Navigate to the ``Save As'' menu. \\
\item Click ``More options''.  The standard Windows ``Save As'' dialogue box will appear. \\
\item Select ``PDF (*.pdf)'' as the file type. Immediately underneath a new ``Options...'' button will appear. \\
\item Click ``Options...'' \\
\item Ensure ``PDF/A compliant when not encrypted via Microsoft Information Protection'' is checked. \\
\end{itemize}
On completion of these steps, PDF/A will become the standard PDF type when creating PDFs from within MSWord. Presently the specific version is PDF/A 3a with no option to select other variants but this may change with future updates to MSWord.
...
\\
\\In the version notes add a comment such as ``Uploaded and approved by \textit{<persons name>} (Worker)''.
\begin{center}
...
\\
\\Check in the document using the same menu structure while adding the comment  ``Approved by \textit{<person name>} (Checker)'' to the version history during check in.
\\
...
\begin{quote}
\texttt{<section name>\_<section report number>\_<year>\_file<file number>.<extension>}
\end{quote}
...
Within Adobe Pro, navigate to \texttt{All Tools>View More>Apply PDF standards} \\
Select ``Apply PDF/A standards''.  \\
Select ``Settings''.  \\
Select ``Save as PDF/A-3a''.\\
Click ``OK'' \\
Note:  Over time as updates are made to Adobe Pro, or if individual users have different configurations, these instructions may not be exactly correct.

\item Chief Metrologist (or delegate) generates a MD5 checksum and adds to the file properties in SharePoint \\
...
Note 1:  This process has assumed the minimum of 3 people signing the report.  In some instances, there can be additional approvers to the report.  This is allowed by checking out the document, adding the appropriate approval comment and checking back in.  This is similar to step 5 above.
...
\item the Chief Metrologist (or delegate) adds a comment to the reissued reports spreadsheet.  Presently this is stored under IRF 378 in the SharePoint BPA-MSL Quality Management System site.
\end{itemize}
The reissued report must remain in the active folder unless it is cancelled at some future date.

\end{enumerate}

\paragraph{Note} In general, changes to the SharePoint BPA-MSL Quality Management System site should only be carried out by the Quality Manager, MQC Members or Chief Metrologist delegates. The procedure for signing electronic reports is an exception to this rule. 
...
When a report has been lost, a replacement may be issued. Simply despatch a copy of the report to the client along with the MD5 in a similar way to section 6.1.5 above.
