...
\item for IANZ-endorsed reports, at least one signatory \proposed{to the report has KTP status under the relevant class};
...
\subsubsection{Issuing Paper Reports}
...
\subsubsection{Issuing Electronic Reports}
...
\end{enumerate}
\proposed{
\subsubsection{Issuing Summary Data}
...
Clients may request calibration information in electronic format, such as an Excel spreadsheet. Such information may be provided, but only after the report has been issued. The electronic data must be checked with the same care given to written reports. Moreover, it should be made clear to the client that the written report (paper or electronic) remains the authoritative document.

\subsubsection{Placing calibration labels on equipment}
While not the default, it is allowable to add calibration labels to equipment.  In such instances the following information must be included on the label
\begin{itemize}
\item The name of the accredited calibration laboratory and its accreditation number
\item The equipment identification
\item The date of the calibration performed at MSL
\item Cross reference to the calibration report issued in respect of the calibration
\end{itemize}

Such labels may carry the IANZ Accredited Laboratory symbol but only when the work has been carried out under MSL’s scope of accreditation \cite{IANZ_Specific_Criteria}.

Note:  In some instances, MSL places identification labels on equipment that does not have a serial number.  The calibration label is distinct from that and thus has different requirements.
}
...
The MSL Publications library is the common repository for publications. It is a folder within the `MSL Quality Management System' library in \proposed{SharePoint}. Various categories of document can be stored there, including: 
\begin{itemize}
...
A Publication Clearance Form can be used to record the review of a publication in the repository. The \proposed{latest version of the form is available within SharePoint by navigating to the 'Forms1' subfolder within the 'Quality Manual' directory.  This form should be uploaded to the relevant subfolder from the 'MSL Publications' section of the central file system.  If not already done so, a new subfolder will need to be created for the publication.}

The version control system in \proposed{SharePoint} keeps track of the different individuals who contribute to the clearance process (e.g., a Team Leader can save comments about IP issues before, or after, a technical review by someone in the team). Extra rows can be added to the form for authors or reviewers (a table structure is used in the form).
...
\proposed{Due to the shutdown of the Callaghan Innovation library service, it is no longer possible to issue new Callaghan Innovation reports.  Until a new process is created to replace this, if you require to issue something similar to a Callaghan Innovation report, please speak to the Quality Manager.}

\paragraph{Technical Guides}

\begin{itemize}
\item \proposed{ Templates for Technical Guides are kept with the current guide documents on the MSL-ORG SharePoint site (look for: }
\verb|Team\Metrology Services\Technical Guides|).
