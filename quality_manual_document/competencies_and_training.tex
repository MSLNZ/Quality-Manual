...
KTP competency will be approved by the Quality Manager.  \proposed{To obtain their first KTP status, the KTP applicant will typically have a meeting with the Quality Manager to assess understanding of MSL's Quality System, ISO/IEC 17025 and relevant IANZ guideline and specific criteria documents.}
...
\begin{itemize}
\item The mentor will provide the Quality Manager with a summary of the individual's training and experience in support of the competency claim ( in some cases the Chief Metrologist, rather than a mentor, will validate evidence in support of a competency and present it to the Quality Manager, see~\S\ref{ss:mentoring} ). \proposed{This evidence shall be submitted using the latest version of the form MSLF.012 Application for new competencies, available from within the Quality Manual section of the SharePoint site.  Usually this form will be updloaded as a PDF to the 'Quality Manager Inbox' section of the SharePoint site, followed by an email to msl-qualitymanager\@measurement.govt.nz to notify the Quality Manager it is there and ready to be processed.}

When requesting KTP competency, a mentor is not required. The applicant shall apply to the Quality Manager directly and provide the necessary evidence of competency.  \proposed{Note if it is a first KTP status, a CV and interview with the Quality Manager will be required (see section 5.3).}

\item The Quality Manager can authorise the new competency by signing, if a paper document has been provided, or clearly indicating acceptance by some other means for electronic documents \proposed{(Note: There is a strong expectation that paper documents will not usually be used.  There are however some circumstances, such as during document management system migrations, where using paper documentation enables continued effective operation of the Quality Management System.  In such cases where paper is used, these must finally be uploaded as PDFs to the SharePoint site, after which the paper copies can be destroyed.)}

\item \proposed{Once signed by the Quality Manager, the signed copy is shared with the applicant to add to their training record documents}

\item \proposed{The signed copy is also shared with} the Section's usual Chief Metrologist delegate checker, who will make a new entry in the TCM (with status ‘pending')

\item The Quality Manager will verify the TCM change and set the status to ‘approved' in the TCM (a record of this is retained by the \proposed{SharePoint} versioning system)

\item A copy of the documents presented to the Quality Manager in support of the new competency will be saved in the relevant Competency Records folder in \proposed{SharePoint}.
\end{itemize}
...
\item \proposed{the signed copy will be shared with the individual concerned, to be filed in their training record}

\item \proposed{the signed copy will also be provided to the Section's usual Chief Metrologist delegate checker, who will delete the TCM entry and set the status to `pending'} 

\item \proposed{the Quality Manager will verify the change and set the TCM item status to ‘approved' (recorded by the AODocs version control system)}

\item \proposed{Note: for completely removing all competencies for a technical procedure, there will no longer be a row in the spreadsheet in which to record approval.  The approval process should still be followed but noting that in this case the approval status will not be captured in the spreadsheet.  Instead this will rely on the SharePoint version control.}

\item a copy of the documents presented to the Quality Manager in support of the change will be filed for future reference in the relevant Technical Competencies folder within the MSL Quality Management System \proposed{SharePoint} library.
\end{itemize}
