...
\item \deprecated{Another copy is provided to the Section's usual Chief Metrologist delegate checker, who will make a new entry in the TCM (with status ‘pending')}

\item The Quality Manager will verify the TCM change and set the status to ‘approved' in the TCM \deprecated{(a record of this is retained by the AODocs versioning system)}

\item A copy of the documents presented to the Quality Manager in support of the new competency will be saved in the \deprecated{relevant Competency Records folder in AODocs}.
\end{itemize}
