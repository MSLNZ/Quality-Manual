...
Both the CIPM \cite{CIPM_MRA_POLICY} and ILAC \cite{ILAC_MRA_POLICY} MRA policies insist on metrological traceability. However, the CIPM MRA is more restrictive about how traceability shall be obtained. In general, it requires external measurements to be obtained from NMIs or DIs. An exception is permitted, however, for \textit{`auxiliary influence quantities, not part of the main traceability path to the SI for a particular measurand'} if the contribution to uncertainty is `minor' (footnote: {A quantitative definition of \textit{minor} is not given.}). In such cases \textit{`measurement services provided by laboratories accredited by a signatory to the ILAC Arrangement'} may be used.
%The CIPM policy gives no objective reason for excluding other ILAC-accredited laboratories from a traceability chain.

A capability published in MSL's IANZ scope of accreditation implies conformance with ILAC MRA policy; a CMC published in the KCDB implies conformance with CIPM MRA policy. The former is a requirement of the latter but not all CMCs . So, MSL reports may be issued with an IANZ endorsement logo on the cover but not the CIPM logo. Doing so indicates that the measurement service is not a KCDB-registered CMC.



\subsubsection{Traceability policy}
 \label{sss:traceability_policy}
The CIPM policy applies to all KCDB-registered CMCs and is the preferred policy for all MSL capabilities.

The criterion for being an `auxiliary influence' is interpreted as follows. If $u_\mathrm{cmc}$ is the nominal \textit{standard uncertainty} associated with a claimed capability, then the maximum acceptable standard uncertainty of an `auxiliary' influence factor shall not exceed $u_\mathrm{cmc} / 10$. 
