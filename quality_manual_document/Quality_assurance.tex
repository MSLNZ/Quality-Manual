...
Metrological traceability policies in the CIPM MRA \cite{CIPM_MRA_POLICY} and ILAC MRA \cite{ILAC_MRA_POLICY} are different. The CIPM MRA is more restrictive, requiring that external measurements be obtained from other NMIs or DIs. The only exception is permitted for \textit{`auxiliary influence quantities, not part of the main traceability path to the SI for a particular measurand'} if the contribution to uncertainty is `minor' (a definition of \textit{minor} is not given). In such cases, \textit{`measurement services provided by laboratories accredited by a signatory to the ILAC Arrangement'} may be used.
...
If a measurement capability is not published in the BIPM KCDB, then compliance with the ILAC MRA traceability policy will be sufficient for entries in our IANZ scope of accreditation. However, the status of MSL as an NMI in the MRA means that the most, if not all, measurement capabilities should strive to meet CIPM expectations. A justification for any exception should be recorded (for example, as a note in the technical procedure).
