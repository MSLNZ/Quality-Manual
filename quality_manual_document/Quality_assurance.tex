...
The policies for metrological traceability in the CIPM MRA \cite{CIPM_MRA_POLICY} and ILAC MRA \cite{ILAC_MRA_POLICY} are different. The CIPM MRA is more restrictive, requiring external measurements to be obtained from other NMIs or DIs. An exception is only permitted for \textit{`auxiliary influence quantities, not part of the main traceability path to the SI for a particular measurand'} if the contribution to uncertainty is `minor' (but a quantitative definition of \textit{minor} is not given). In such cases, \textit{`measurement services provided by laboratories accredited by a signatory to the ILAC Arrangement'} may be used.

A capability published in the BIPM KCDB must conform to the CIPM MRA policy.

\subsubsection{Traceability policy}
 \label{sss:traceability_policy}
The CIPM MRA traceability policy is preferred for all MSL capabilities.
...
When a capability is not published in the BIPM KCDB, ILAC MRA traceability policy may apply. Justification for the exception to MSL policy should be recorded (for example, as a note in the technical procedure).
